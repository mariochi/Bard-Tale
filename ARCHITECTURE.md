...
- **Mixer App** (`ApplicationV2` + `HandlebarsApplicationMixin`): 3 strips verticais (Background/Overlay/Theme) — now playing, dropdown de duas camadas pra cada layer (GM/DJ): escolher a **playlist inteira** (carrega a primeira faixa, mantém a rotação normal — repetir/embaralhar conforme configurado na Library) ou, dentro do `<optgroup>` de cada playlist, uma **faixa específica** (`playlistId::trackId` como valor do `<option>`) — toca só aquela faixa, isolada, e para ao terminar (`AudioEngine#applyRemoteLoadLayer` com `standalone: true`, não anexa `controller.playlist`, então `_onTrackEnded` não acha próxima faixa e chama `stop()` sozinho — zero código novo nesse ponto). Transporte (só GM/DJ vê botões habilitados), fader de volume (visível a todos, local).
...
- **Floating Menu** (`FloatingMenu.mjs`): menu fixo na tela, fora do sistema de Scene Controls, injetado direto em `document.body` no `ready` — dois botões DOM comuns (Mixer/Library) com `addEventListener('click', ...)` de verdade, sem depender de nenhum mecanismo especial do Foundry. Posição: canto inferior esquerdo, acima da caixinha de status do jogador (`#players`) — ver histórico completo da saga dos 3 lugares testados na seção 9.
...
- **Histórico completo de onde colocar os botões de abrir Mixer/Library (3 tentativas):**
  1. Botões pendurados dentro do grupo nativo "tokens" — funcionava, mas ficava escondido atrás de entrar na categoria de Tokens primeiro (ruim pra descoberta).
  2. Menu flutuante (`FloatingMenu.mjs`) — não apareceu em tela na primeira tentativa (bug de posição: `left:64px/bottom:12px` caía embaixo da caixinha de status do jogador, `#players`); corrigido pra `left:10px/bottom:70px`, mas nunca voltamos a testar esse fix porque migramos pra tentativa 3 antes.
  3. Grupo próprio nos Scene Controls (`bardTale`) — testamos contra o bug conhecido do core pra grupos novos ([foundryvtt/foundryvtt#12258](https://github.com/foundryvtt/foundryvtt/issues/12258)) e não reproduziu na hora. Só que meses depois surgiu outro problema: o botão ficava "travado apertado" visualmente e parava de reabrir a janela depois do primeiro clique. Suspeita (não 100% confirmada pela doc oficial): `activeTool: 'openMixer'` no `SceneControl` apontava pra um tool com `button: true`, e a doc do `SceneControlTool.active` diz explicitamente "Not applicable to toggles or buttons" — ou seja, `activeTool` nunca deveria apontar pra um botão. É provável que isso deixe o Foundry tratando aquele botão como permanentemente "selecionado" do grupo.
  
  **Decisão final: voltamos pra tentativa 2** (Floating Menu), já com a posição corrigida — botão DOM comum não tem ambiguidade nenhuma sobre disparar o clique toda vez, ao contrário do mecanismo de tools dos Scene Controls, que se mostrou frágil duas vezes em pontos diferentes.
...
- **(Corrigido) Falha ao restaurar uma camada no `ready` travava o Mixer/Library pra sempre.** `AudioEngine#resumeFromSnapshot()` rodava sem try/catch, antes de `game.bardTale.mixerApp`/`libraryApp` serem criados no hook `ready`. Uma faixa persistida que falhasse ao carregar (ex.: arquivo local movido/removido do servidor entre sessões) abortava o `resumeFromSnapshot` inteiro, e como as duas Apps são criadas DEPOIS dessa chamada, elas nunca chegavam a existir — toda tentativa de abrir o painel falhava silenciosamente (`game.bardTale.mixerApp` undefined), em todo reload subsequente, sem nenhum jeito de se recuperar sem mexer direto no console. Corrigido em duas camadas: try/catch por camada dentro de `resumeFromSnapshot` (uma falha isolada não derruba as outras) e try/catch ao redor da chamada inteira em `main.mjs` (garante que Mixer/Library sempre são criados, aconteça o que acontecer na restauração).
...
- **(Corrigido) `stop()` só pausava, não destruía o player — sujava o estado.** Sintoma relatado: faixa local curta no overlay, "stop", e o nome da faixa continuava mostrando + "play" depois não tocava nada. Causa: um `AudioBufferSourceNode` do Web Audio API é descartável — depois de terminar (naturalmente ou pausado num estado ruim), não dá pra simplesmente mandar `.play()` de novo nele, diferente do YouTube, cujo `playVideo()` consegue reiniciar o mesmo vídeo já terminado sem problema. `LayerController#stop()` (renomeado de `clearCurrentTrack`) agora destrói o handle de verdade (`_playerHandle = null`, `_providerId = null`), não só pausa — usado tanto no "stop" explícito (`AudioEngine#applyRemoteStopLayer`) quanto quando uma playlist termina sem próxima faixa (`_onTrackEnded`). Comportamento uniforme pros dois providers de propósito (era assim que já funcionava, incidentalmente, só pro YouTube).
